...
3. Enter a destination name **ACI\_SAP\_EM** and select the connection type **G Http connection to External Server** from the popup which appears once you enter **G** in that field, select the **Save** icon.
...
10. Enter a destination name **ACI\_SAP\_EM\_TOKEN** and choose connection type **G Http connection** from the drop down, select the **Save** icon.
...
3. In the screen, **Maintain the Cloud Shared Secret**:
   ...
   - Select **Execute** ( You will see a success Message---> **Shared Secret for ACI\_SAP\_EM\_CAL was created successfully!**)
